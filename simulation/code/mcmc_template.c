...
{
    if (argc < 5) {
        printf("\nPlease provide label of the data set, index of simulation data, index of mcmc chains, and label of case sampler (1: element-wise random walk; 2: block sampling) as extra arguments!\n");
        return 0;
    }
    
    char* data_label, *Y_sampler_string;
    int index_epi, index_chain, Y_sampler_code;
    data_label = argv[1];
    index_epi = atoi(argv[2]);
    index_chain = atoi(argv[3]);
    Y_sampler_code = atoi(argv[4]);
    Y_sampler_string = Y_sampler_code==1 ? "element" : "block";
	printf("Data: %s\n", data_label);
    printf("Data index: %d\n", index_epi);
	...
	int r,s,t,v,i,j,k, index_r, index_nb, index_j, index_t, index_k, index_v;
    ...
    int new_Y, new_Y_nv, old_Y, old_Y_nv, max_step=10, step_size;
    ...
    /* Read Yv */
    sprintf(filename, "simulation/sim_data/south5_Yv_simdata_rep%d.txt", index_epi);
    ...
    /* Read Z */
    sprintf(filename, "simulation/sim_data/south5_Z_simdata_%s_rep%d.txt", data_label, index_epi);
    file = fopen(filename, "r");
    if (file == NULL)
    {
    ...
    for (r=0; r < n_region; r++)
    {
        for (s=0; s < n_s; s++)
        {
            for (t=0; t < n_week; t++)
            {
                ...
                for (v=0; v < n_v-1; v++)
                {
                    polygons[r].Y[s][t][v] = polygons[r].Z[s][t][v] + (polygons[r].Yv[s][t]-sum_Z)/3;
    ...
    while (adapt_status == 0 && count_batch < max_batch)
    {
        ...
        for (i=0; i < n_batch; i++)
        {
            ...
            {    
                ...
                logacprate = -1.0 * alphas_logdensity_fun(polygons,v,gamma0,gamma1,gamma2,sigma2_alpha, eta_E, eta_S);
                ...
                logacprate = logacprate + alphas_logdensity_fun(polygons,v,gamma0,gamma1,gamma2,sigma2_alpha, eta_E, eta_S);
            ...
            for (v=0; v < n_v; v++)
            {
                shape_sigma2_alpha = (n_region - 3)/2;
                rate_sigma2_alpha = 0;
                for (index_r=0; index_r < n_region; index_r++)
                {
            ...
            for (v=0; v < n_v; v++)
            {
                ...
                logacprate = -1.0 * beta_fixs_logdensity_fun(polygons,v,gamma0,gamma1,gamma2,beta_fix,beta_fix_mean,beta_fix_var, eta_E, eta_S);
                ...
                logacprate = logacprate + beta_fixs_logdensity_fun(polygons,v,gamma0,gamma1,gamma2,beta_fix,beta_fix_mean,beta_fix_var, eta_E, eta_S);
            ...
            for (v=0; v < n_v; v++)
            {
                ...
                {
                    logacprate = -1 * gamma0_logdensity_fun(polygons, v, gamma0, gamma1, gamma2, shape0, rate0, eta_E, eta_S);
                    gamma0[v] = new_gamma0;
                    logacprate = logacprate + gamma0_logdensity_fun(polygons, v, gamma0, gamma1, gamma2, shape0, rate0, eta_E, eta_S);
                ...
                {
                    logacprate = -1 * gamma1_logdensity_fun(polygons, v, gamma0, gamma1, gamma2, shape1, rate1, eta_E, eta_S);
                    gamma1[v] = new_gamma1;
                    logacprate = logacprate + gamma1_logdensity_fun(polygons, v, gamma0, gamma1, gamma2, shape1, rate1, eta_E, eta_S);
                ...
                {
                    logacprate = -1 * gamma2_logdensity_fun(polygons, v, gamma0, gamma1, gamma2, shape2, rate2, eta_E, eta_S);
                    gamma2[v] = new_gamma2;
                    logacprate = logacprate + gamma2_logdensity_fun(polygons, v, gamma0, gamma1, gamma2, shape2, rate2, eta_E, eta_S);
           ...
           // Y; random walk
            if (Y_sampler_code == Y_RANDOM)
            {
                for (t=1; t < n_week-1; t++)
                {
                    for (s=0; s < n_s; s++)
                    {
                        for (r=0; r < n_region; r++)
                        {
                            ...
                            {
                                // Find the range of Y
                                minY = polygons[r].Z[s][t][v];
                                maxY = maxY_fun(r,s,t,v,polygons);
                                
                                if (minY == maxY)
                                {
                                    polygons[r].Y[s][t][v] = minY;
                                    continue;
                                }
                                
                                step_size = ceil(max_step*runiform(&seed));
                                old_Y = polygons[r].Y[s][t][v];
                                old_Y_nv = polygons[r].Y[s][t][n_v-1];
                                if (runiform(&seed) < 0.5)
                                {
                                    new_Y = old_Y - step_size;
                                    new_Y_nv = old_Y_nv + step_size;
                                }
                                else
                                {
                                    new_Y = old_Y + step_size;
                                ...
                                if (new_Y < minY || new_Y > maxY) continue;
                                
                                logacprate = logpmfY_fun(old_Y, polygons,r,s,t,v,gamma0,gamma1,gamma2,eta_E,eta_S) + logpmfY_fun(old_Y_nv, polygons,r,s,t,n_v-1,gamma0,gamma1,gamma2,eta_E,eta_S);
                                logacprate = -1.0*logacprate + logpmfY_fun(new_Y, polygons,r,s,t,v,gamma0,gamma1,gamma2,eta_E,eta_S) + logpmfY_fun(new_Y_nv, polygons,r,s,t,n_v-1,gamma0,gamma1,gamma2,eta_E,eta_S);
                                
                                if (runiform(&seed) < exp(logacprate))
                                {
                                    polygons[r].Y[s][t][v] = new_Y;
                                    polygons[r].Y[s][t][n_v-1] = new_Y_nv; // update Y
                                    polygons[r].Ys[t][v] = polygons[r].Ys[t][v] - old_Y + new_Y;
                                    polygons[r].Ys[t][n_v-1] = polygons[r].Ys[t][n_v-1] - old_Y_nv + new_Y_nv; // update Ys
                                }
                            }
                        }
                    }
           ...
           // block sampling; multinomial proposal
                ...
                for (t=1; t < n_week-1; t++)
                {
                    for (s=0; s < n_s; s++)
                    {
                        for (r=0; r < n_region; r++)
                        {
                            ...
                            for (v = 0; v < n_v; v++)
                            {
                                pmf_Y[v] = polygons[r].ps[s][t][v] * lambda_fun(polygons, r, t, v, gamma0, gamma1, gamma2, eta_E, eta_S);
                                sum_pmf_Y += pmf_Y[v];
                            }
                            ...
                            logacprate = 0.0;
                            for (v = 0; v < n_v; v++) logacprate += logpmfY_fun(old_Y_block[v],polygons,r,s,t,v,gamma0,gamma1,gamma2,eta_E,eta_S) - (old_Y_block[v] - polygons[r].Z[s][t][v]) * log(pmf_Y[v]) + lgamma(old_Y_block[v] - polygons[r].Z[s][t][v] + 1);
                            
                            logacprate = -1.0*logacprate;
                            
                            for (v = 0; v < n_v; v++) logacprate += logpmfY_fun(new_Y_block[v],polygons,r,s,t,v,gamma0,gamma1,gamma2,eta_E,eta_S) - (new_Y_block[v] - polygons[r].Z[s][t][v]) * log(pmf_Y[v]) + lgamma(new_Y_block[v] - polygons[r].Z[s][t][v] + 1);
            ...
            for (v=0; v < n_v; v++)
            {
                ...
                for (index_r=0; index_r < n_region; index_r++)
                {
                    ...
                    {
                        lambda_old = lambda_fun(polygons, index_r, index_t, v, gamma0, gamma1, gamma2, eta_E, eta_S);
                        for (index_j=0; index_j < n_cov; index_j++) eta_S[index_j][v] = new_eta_S[index_j];
                        lambda_new = lambda_fun(polygons, index_r, index_t, v, gamma0, gamma1,gamma2, eta_E, eta_S);
            ...
            for (v=0; v < n_v; v++)
            {
                ...
                logacprate = -1.0 * eta_E_logdensity_fun(polygons,v,gamma0,gamma1,gamma2,eta_E,eta_S,eta_E_mean,eta_E_var);
                ...
                logacprate = logacprate + eta_E_logdensity_fun(polygons,v,gamma0,gamma1,gamma2,eta_E,eta_S,eta_E_mean,eta_E_var);
    ...
    for (i=0; i < n_sim + n_burn; i++)
    {
        ...
        for (v=0; v < n_v; v++)
        {
            shape_sigma2_alpha = (n_region - 3.0)/2;
            rate_sigma2_alpha = 0;
            for (index_r=0; index_r < n_region; index_r++)
            {
                for (index_nb=0; index_nb < polygons[index_r].n_nb; index_nb++)
        ...
        {
            sprintf(filename, "simulation/output_chains/Y_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            ...
            sprintf(filename, "simulation/output_chains/gamma0_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            if (file == NULL)
            {
            ...
            sprintf(filename, "simulation/output_chains/gamma1_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            if (file == NULL)
            {
            ...
            sprintf(filename, "simulation/output_chains/gamma2_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            if (file == NULL)
            {
            ...
            sprintf(filename, "simulation/output_chains/alpha_element_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            if (file == NULL)
            {
            ...
            sprintf(filename, "simulation/output_chains/sigma2_alpha_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            if (file == NULL)
            {
            ...
            sprintf(filename, "simulation/output_chains/beta_fix_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            if (file == NULL)
            {
            ...
            sprintf(filename, "simulation/output_chains/eta_P_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            ...
            sprintf(filename, "simulation/output_chains/eta_P0_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            ...
            sprintf(filename, "simulation/output_chains/eta_S_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
            ...
            sprintf(filename, "simulation/output_chains/eta_E_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
            file = fopen(filename, "a");
    ...
    sprintf(filename, "simulation/output_chains/acceptance_rate_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
    file = fopen(filename, "a");
    fprintf(file, "gamma0\n");
   ...
   for (v=0; v < n_v; v++) fprintf(file, "v=%d %f\n", v, (double)n_accept_alphas[v]/(n_sim + n_burn));
    ...
    sprintf(filename, "simulation/output_chains/tuning_para_output_simdata_%s_%s_%d_%d.txt", data_label, Y_sampler_string, index_epi, index_chain);
    file = fopen(filename, "a");
    fprintf(file, "gamma0\n");
    for (v=0; v < n_v; v++) fprintf(file, "v=%d %f\n", v, tuning_gamma0[v]);
