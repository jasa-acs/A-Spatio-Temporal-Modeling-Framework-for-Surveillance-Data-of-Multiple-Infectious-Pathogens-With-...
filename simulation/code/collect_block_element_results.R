...
for (index.epi in 1:nepi)
{
	for (index.para in 1:npara)
	{
		...
		{
			if (index.para == 1) load(paste("simulation/output_chains/south5_simdata_real_element_", index.epi, "_", index.chain, "_results.RData", sep=""))
			else load(paste("simulation/output_chains/south5_simdata_real_block_", index.epi, "_", index.chain, "_results.RData", sep=""))
...
save(bmse_res, stat_res, file="simulation/results_summary/summary_simulation_block_element_results.RData")
